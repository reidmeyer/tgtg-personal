...
def is_approximately_third(A, B, tolerance=10):
    """
    Check if A is approximately one-third of B within a given tolerance.
    """
    target = B / 3
    return abs(A - target) <= tolerance
            ...
            for item in items:
                if item["items_available"] > 0:
                    if item["item"]["item_id"] not in items_notified:
                        if is_approximately_third(
                            item["item"]["item_price"]["minor_units"],
                            item["item"]["item_value"]["minor_units"],
                        ):  # Then there is no dynamic pricing. lowest price it will go
